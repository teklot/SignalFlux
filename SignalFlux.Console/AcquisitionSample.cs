    ...
    public static class AcquisitionSample
    {
        public static async Task RunAsync()
        {
            ...
            try
            {
                ...
                server.Start();
                WriteLine($"Sensor server listening on port {port}...");

                var sensorTask = SimulateSensorAsync(port);
                ...
                if (values.Count > 0)
                {
                    ...
                    await using (var sqliteRead = new SqliteSignalStore(sqlitePath, createNew: false))
                    {
                    ...
                    WriteLine();
                    WriteLine("Pipeline: Simulated Sensor -> TCP -> Signal -> CSV + SQLite");
                }
                else
                {
